...
import { DashboardCustomerDetailsRequest,DashboardCustomerDetailsResponse,FundAccountRequest,FundAccountResponse } from '../../../services/DashboardModel';
...
export class DashboardComponent implements OnInit {
  ...
  @Input() Obj={accountNo:localStorage.getItem("AccountNo")}
  @Input() Obj2={amount:"0"}
  @Input() FundObj={RetrievalReference:"",AccountNumber:"",Amount:0,Narration:""}
  ...
  constructor(private calendar: NgbCalendar,public ds:DashboardService,private modalService: NgbModal) {
  ...
  LoadPayStack(){
    localStorage.setItem("Amount",this.Obj2.amount);
    new payWithPaystack();
  }
  DoAmount($event){
   
    this.Obj2.amount=$event.target.value;
    this.transAmount=$event.target.value*100;
  }
  ...
  FetchTransactions(){
    this.ds.TransHistory(this.Obj).subscribe(d=>{
      this.trans=d;
    },(error)=>{

    })
  }
  RetrievedashboardDetails(data:DashboardCustomerDetailsRequest){
    
    this.ds.CustomerDetails(data).subscribe(d=>{
      if(d.responseCode =="00"){
        this.AvailableBalance=d.AvailableBalance;
        this.WithdrawableBalance=d.WithdrawableBalance;
        this.AccountType=d.AccountType;
        this.AccountNumber=this.Obj.accountNo;
      }else{
        this.AvailableBalance="NA";
        this.WithdrawableBalance="NA";
      }
    },error=>{
//popup toast
console.log(error);
  ...
  ngOnInit(): void {
    //this.reference = 'ref-${Math.ceil(Math.random() * 1013)}';
    this.reference=this.randomInteger(1000000,10000000);
    this.RetrievedashboardDetails(this.Obj);
  ...
  paymentDone(ref: any) {
    ...
    this.FundObj={
      RetrievalReference:this.reference,
      AccountNumber:this.Obj.accountNo,
      Amount:this.transAmount,
      Narration:"Funding of Account"
    }
    
    this.ds.CreditCustomersAccount(this.FundObj).subscribe(d=>{
      if(d.responseCode =="00"){
        this.RetrievedashboardDetails(this.Obj);
        this.FetchTransactions();
